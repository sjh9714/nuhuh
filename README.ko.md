...
<p align="center">
  <img src="docs/demo.gif" width="880" alt="nuhuh 데모. 에이전트가 완료를 선언하면 nuhuh가 모든 주장을 신선하게 재실행해 거짓 완료를 빨간 영수증과 함께 반송하고, 수정 후 영수증이 초록이 되면 통과시킵니다">
</p>
...
기본값은 초록 영수증일 때 조용히 통과입니다. `NUHUH_RECEIPT=always`를 켜면
성공 영수증도 세션에 출력해서, 에이전트가 자신감 있는 문장 하나로 충분하다고
학습하지 않게 합니다.
`NUHUH_STRICT=1`은 한 걸음 더 나가서, 체크 가능한 주장이 하나도 없는 완료
선언을 반송하고 증거를 요구합니다. 주장 없는 "완료했습니다!"가 실측에서
가장 흔한 거짓 완료 유형이었습니다.

## 위임할수록 필요해집니다

우리 벤치마크가 말해주는 것. 프론티어 모델은 완료에 대해 거의 거짓말하지
않습니다(102런에서 0.0%). 거짓 완료는 작고 싼 모델에서 나왔습니다(Haiku 4.5
6.0%, Codex 2.1%). 그리고 에이전트 워크플로는 정확히 그쪽으로 가고 있습니다.
강한 오케스트레이터 하나가 싼 워커들에게 위임하고, 배치가 돌고, 아무도 안
보는 CI가 돕니다. 게이트는 그 위임 차선의 안전벨트입니다. 싼 작업은 기계로
검증하고, 비싼 모델은 판단에 쓰세요.
...
| "변경사항을 커밋했습니다" | git을 읽어서, 추적 중인 파일에 커밋 안 된 변경이 남아 있으면 주장을 실패시킵니다 |
...
- `nuhuh init`이 쓰는 훅은 설치한 그 버전으로 고정됩니다. 게이트가 정지 시점에 가변 코드를 받아오는 일이 없습니다. 새 버전으로 올리는 것은 `nuhuh uninit` 후 `npx nuhuh@<버전> init`이라는 명시적 행동입니다
- `nuhuh init`은 설정 파일을 먼저 백업하고, `nuhuh uninit`은 어떤 버전으로 고정됐든 훅 항목을 깨끗이 되돌립니다
