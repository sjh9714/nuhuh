...
대조합니다. 계정도, API 키도, **모델 호출도 없습니다.** 아무것도 컴퓨터
밖으로 나가지 않습니다.
...
주장 매칭은 현재 영어와 한국어를 지원합니다. 패턴은
[데이터 파일](src/claims/patterns.ts)이라서 언어 추가는 포크가 아니라 PR입니다.

## 왜 LLM에게 검사를 안 시키나

측정됐고, 못 하기 때문입니다. 판사 모델 5종과 프롬프트 전략 5종 전부에서 LLM
판사의 거짓 완료 탐지력은 [**AUROC 0.54에서 0.65**](https://arxiv.org/abs/2606.09863),
...
**검증 경로에 LLM 호출 0회, 결정론적, 같은 세션이면 같은 영수증.**
...
Node 20 이상. Claude Code 세션은 `~/.claude/projects`에서, Codex 롤아웃은
