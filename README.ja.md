...
ということです(102ランで0.0%)。偽の完了は小さく安いモデルから出ました
(Haiku 4.5 6.0%、Codex 2.1%)。そしてエージェントのワークフローは、まさに
...
| 「変更をコミットしました」 | git を読み、追跡中のファイルに未コミットの変更が残っていれば主張を失敗にします |
...
- `nuhuh init` が書くフックは、インストールしたバージョンに固定されます。ゲートが停止時に可変なコードを取りに行くことはありません。新しい版へ上げるのは `nuhuh uninit` のあと `npx nuhuh@<version> init` という明示的な操作です
- `nuhuh init` は設定ファイルを先にバックアップし、`nuhuh uninit` はどのバージョンに固定されていてもフック項目をきれいに戻します
